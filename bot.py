...
AIRDROP_AMOUNT = "{:,}".format(float(AIRDROP_AMOUNT))
...
⭐️ For each referral - Get *{"{:,}".format(REFERRAL_REWARD)} {COIN_SYMBOL}*
...
🔹 Extra reward per referral = *{"{:,}".format(REFERRAL_REWARD)} {COIN_SYMBOL}*
...
Your personal referral link (+{"{:,}".format(REFERRAL_REWARD)} {COIN_SYMBOL} for each referral)
REPLACEME
"""

WITHDRAWAL_TEXT = f"""
📌You must have at least 1 referral
...
def loopAnswer(update, context):
    ...
    reply = ""
    if(message == "💰 Balance"):
        refbal = "{:,}".format(info["refCount"]*REFERRAL_REWARD)
        if(refbal == ""):
            refbal = "0"
...
A total of *{"{:,}".format(float(AIRDROP_AMOUNT.replace(",",""))*list.count())} {COIN_SYMBOL}* will be distributed as participation rewards
A total of *{"{:,}".format(REFERRAL_REWARD*refes)} {COIN_SYMBOL}* referral rewards will be distributed
"""
